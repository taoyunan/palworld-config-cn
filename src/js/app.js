...
function refreshChangedRows() {
  ...
  changedCount.textContent = `${changed} 项已修改`;
}
...
function buildOptionSettings() {
  return FIELDS.map((field) => `${field.key}=${formatIniValue(field, state.get(field.key))}`).join(",");
}

function buildIni() {
  return [
    "; This configuration file was generated by the Chinese Palworld server configuration page.",
    "; Copy it to Pal/Saved/Config/WindowsServer/PalWorldSettings.ini or Pal/Saved/Config/LinuxServer/PalWorldSettings.ini.",
    "[/Script/Pal.PalGameWorldSettings]",
    `OptionSettings=(${buildOptionSettings()})`,
    ""
  ].join("\n");
}
...
function applyImportedText(text) {
  ...
  parsed.forEach((value, key) => setValue(key, value, true));
  updateOutput();
  importDialog.close();
  showToast(`已导入 ${parsed.size} 项配置。`);
}
...
function downloadIni() {
  const blob = new Blob([buildIni()], { type: "text/plain;charset=utf-8" });
...
document.querySelector("#copyOptionBtn").addEventListener("click", () => copyText(`OptionSettings=(${buildOptionSettings()})`, "OptionSettings 已复制。"));
document.querySelector("#selectAllBtn").addEventListener("click", () => {
  outputText.focus();
  outputText.select();
});
document.querySelector("#downloadBtn").addEventListener("click", downloadIni);
